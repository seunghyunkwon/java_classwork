...
//	ATriangle t = (ATriangle)shapes[1];	//오류 : 만들어진 객체와 다른 타입으로 캐스팅
	ATriangle t = (ATriangle)shapes[2];	//실행 : 만들어진 객체와 참조 타입이 일치
	
	for(int i=0;i<shapes.length;i++) {
		ATriangle at;
		ASquare as;
		if(shapes[i] instanceof ATriangle) {
				//shapes[i]가 참조하는 객체가 ATriangle 객체인지 검사한다 (instanceof 연산자)
			at = (ATriangle)shapes[i];
			at.setWidth(100); at.setHeight(50);
			System.out.println(at.triangle());
		}else if(shapes[i] instanceof ASquare) {
				//shapes[i]가 참조하는 객체가 ASquare 객체인지 검사한다 (instanceof 연산자)
			as = (ASquare)shapes[i];
			as.setWidth(90);
